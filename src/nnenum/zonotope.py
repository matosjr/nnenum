...
def zono_from_compressed_init_box(init_bm, init_bias, init_box):
    ...
    for index, (lb, ub) in enumerate(init_box):
        vec = np.array(
            [1 if d == index else 0 for d in range(len(init_box))], dtype=dtype
        )
...
class Zonotope(Freezable):
    ...
    def __init__(self, center, gen_mat_t, init_bounds=None):
        ...
        if center is not None:
            ...
            assert (
                len(center.shape) == 1 or center.shape[0] == 1
            ), f"Expected 1-d center, got {center.shape}"
        else:
        ...
        if gen_mat_t is None:
            ...
            assert (
                len(gen_mat_t.shape) == 2
            ), f"expected 2-d gen_mat_t, got {gen_mat_t.shape}"
            assert isinstance(gen_mat_t, np.ndarray), f"gen_mat_t was {type(gen_mat_t)}"

            if gen_mat_t.size > 0:
                assert len(self.center) == gen_mat_t.shape[0], (
                    f"center has {len(self.center)} dims but "
    ...
    def __str__(self):
        return (
            f"[Zonotope with center {self.center} and generator matrix_t:\n{self.mat_t}"
    ...
    def contract_lp(self, star, hyperplane_vec, rhs):
        ...
        new_bounds_list = star.update_input_box_bounds(
            hyperplane_vec, rhs, count_lps=True
        )
    ...
    def update_init_bounds(self, i, new_bounds, skip_check=False):
        ...
        if skip_check:
            ...
            assert new_bounds[0] + ep >= self.init_bounds[i][0], (
                f"new lower bound ({new_bounds[0]} was worse "
        ...
        if skip_check:
            ...
            assert new_bounds[1] - ep <= self.init_bounds[i][1], (
                f"new upper bound ({new_bounds[1]}) was worse"
    ...
    def box_bounds(self):
        ...
        if self.pos1_gens is None or self.pos1_gens.shape[0] != self.mat_t.shape[1]:
            self.neg1_gens = np.array(
                [i[0] for i in self.init_bounds], dtype=self.dtype
            )
            self.pos1_gens = np.array(
                [i[1] for i in self.init_bounds], dtype=self.dtype
            )
    ...
    def update_output_bounds(self, layer_bounds, update_indices):
        ...
        if self.mat_t.size == 0:
            ...
            if mat_t.size > 0:
                ...
                for i, pp_row, nn_row, pn_row, np_row in zip(
                    update_indices, pos_pos, neg_neg, pos_neg, neg_pos
                ):
                    ub = pp_row + nn_row
                    lb = pn_row + np_row
                    c = self.center[i]
