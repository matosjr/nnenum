...
class Timers:
    ...
    stack = (
        []
    )  # stack of currently-running timers, parents at the start, children at the end
    enabled = True
    ...
    def toc(name):
        ...
        if Timers.enabled:
            ...
            if not Timers.stack:
                raise RuntimeError(
                    f"Timer '{name}' was stopped, but no timers were running"
                )

            assert (
                Timers.stack[-1].name == name
            ), "Out of order toc(). Expected to first stop timer {}".format(
    ...
    def print_stats_recursive(td, level, total_time, short):
        ...
        total_time = (
            1e-9 if total_time < 1e-9 else total_time
        ...
        if not below_threshold:
            ...
            if td.children:
                ...
                other_print_func(
                    "{}Other ({}): {:.2f} sec{}".format(
                        " " * (level + 1) * 2, td.name.capitalize(), other, percent_str
                    )
                )
