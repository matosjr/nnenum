...
class NeuralNetwork(Freezable):
    ...
    def __init__(self, layers):
        ...
        for i, layer in enumerate(layers):
            assert (
                layer.layer_num == i
            ), f"Layer {i} has incorrect layer num: {layer.layer_num}: {layer}"
    ...
    def __str__(self):
        return (
            f"[NeuralNetwork with {len(self.layers)} layers with {self.layers[0].get_input_shape()} input and "
    ...
    def execute(self, input_vec, save_branching=False):
        ...
        for layer in self.layers:
            if (
                save_branching
                and isinstance(layer, ReluLayer)
    ...
    def check_io(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            assert prev_output_shape == my_input_shape, (
                f"output of layer {i-1} was {prev_output_shape}, "
...
class ReluLayer(Freezable):
    ...
    def __init__(self, layer_num, shape, filter_func=None):
        ...
        self.filter_func = (
            filter_func  # returns True if relu should be applied for neuron i
        )
    ...
    def execute(self, state, save_branching=False):
        ...
        assert state.shape == self.get_input_shape(), (
            f"state shape to fully connected layer was {state.shape}, "
...
class MatMulLayer(Freezable):
    ...
    def __init__(self, layer_num, mat, prev_layer_output_shape=None):
        ...
        assert prev_layer_output_shape is None or isinstance(
            prev_layer_output_shape, tuple
        )
        ...
        if prev_layer_output_shape is not None:
            ...
            assert expected_inputs == mat.shape[1], (
                f"MatMulLayer matrix shape was {mat.shape}, but "
    ...
    def execute(self, state):
        ...
        assert state.shape == self.get_input_shape(), (
            f"state shape to matmul was {state.shape}, "
            + f"expected {self.get_input_shape()}"
        )
...
class FullyConnectedLayer(Freezable):
    ...
    def __init__(self, layer_num, weights, biases, prev_layer_output_shape=None):
        ...
        assert (
            len(biases.shape) == 1
        ), f"expected 1-d bias vector at layer {layer_num}, got {biases.shape}"
        ...
        if prev_layer_output_shape is not None:
            ...
            assert expected_inputs == weights.shape[1], (
                f"FC Layer weight matrix shape was {weights.shape}, but "
                + f"prev_layer_output_shape {prev_layer_output_shape} needs {expected_inputs} columns"
            )
...
class Convolutional2dLayer(Freezable):
    ...
    def __init__(
        ...
        assert isinstance(
            prev_layer_output_shape, tuple
        ), f"prev_layer_shape was {prev_layer_output_shape}"
        ...
        for k in kernels:
            ...
            for channel_kernel in k:
                assert (
                    len(channel_kernel.shape) == 2
                ), "expected a list of list of 2d kernels"
    ...
    def execute(self, state, zero_bias=False):
        ...
        assert state.shape == self.prev_layer_output_shape, (
            f"expected shape {self.prev_layer_output_shape}, " + f"got {state.shape}"
        )
        ...
        for kernel, bias in zip(self.kernels, self.biases):
            ...
            for i, channel_kernel in enumerate(kernel):
                ...
                channel_out = convolve2d(
                    state2d, channel_kernel, mode=self.mode, boundary=self.boundary
                )
...
class PoolingLayer(Freezable):
    ...
    def __str__(self):
        ...
        return (
            f"[PoolingLayer ({self.method}) {s}x{s} with stride {self.stride}, "
                ...
                for col_index in range(state.shape[1]):
                    ...
                    if self.method == "max":
                        ...
                        if val - epsilon > output[output_row, block_index, d]:
                            ...
                            branch_list[
                                depth_offset + height_offset + block_index
                            ] = mindex
                            ...
                            output[output_row, block_index, d] = max(
                                output[output_row, block_index, d], val
                            )
...
def weights_biases_to_nn(weights, biases, dtype=None):
    ...
    assert (
        biases.shape[0] == num_layers
    ), f"nn has {num_layers} layers, but biases shape was {biases.shape}"
