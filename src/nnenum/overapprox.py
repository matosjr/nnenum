...
def check_round(ss, sets, spec_arg, check_cancel_func=None):
    ...
    spec_list = (
        spec_arg.spec_list if isinstance(spec_arg, DisjunctiveSpec) else [spec_arg]
    )
...
class RoundsResult:
    ...
    def __str__(self):
        ...
        if Settings.SAVE_BRANCH_TUPLES_TIMES:
            rv = ", ".join(
                [
                    f"{max(r)} ({round(ms, 1)} ms)"
                    for r, ms in zip(self.round_generators, self.round_ms)
                ]
            )
        else:
    ...
    for vstar, vindex in zip(vstars, vindices):
        ...
        for row in rows:
            ...
            if cur_spec.is_violation(flat_output):
                if Settings.PRINT_OUTPUT:
                    print(
                        "Found unsafe from first concrete execution of abstract counterexample"
                    )
...
def do_overapprox_rounds(
    ...
    for round_num, types in enumerate(overapprox_types):
        ...
        if (
            not ss.branch_tuples
            and Settings.PRINT_OUTPUT
            and Settings.PRINT_OVERAPPROX_OUTPUT
        ):
            print(
                f"Overapprox Round {round_num+1}/{len(overapprox_types)} has {len(sets)} set(s)"
            )
        ...
        try:
            ...
            diff = (
                time.perf_counter() - start if Settings.SAVE_BRANCH_TUPLES_TIMES else 0
            )
        except OverapproxCanceledException as e:
            diff = (
                time.perf_counter() - start if Settings.SAVE_BRANCH_TUPLES_TIMES else 0
...
def run_overapprox_round(network, ss_init, sets, prerelu_sims, check_cancel_func=None):
    ...
    for layer_index, layer in enumerate(remaining_layers):
        ...
        if isinstance(layer, ReluLayer):
            ...
            for s in sets:
                s.execute_with_bounds(
                    layer_num, layer_bounds, split_indices, zero_indices
                )
        else:
            # non-relu layer
...
def make_split_indices(layer_bounds):
    ...
    split_indices = np.nonzero(
        np.logical_and(
            layer_bounds[:, 0] < -Settings.SPLIT_TOLERANCE,
            layer_bounds[:, 1] > Settings.SPLIT_TOLERANCE,
        )
    )[0]
...
class StarOverapprox(Freezable):
    ...
    def execute_with_bounds(self, layer_num, layer_bounds, split_indices, zero_indices):
        ...
        if self.get_num_gens() + len(split_indices) > self.max_gens:
            raise OverapproxCanceledException(
                f"star gens exceeds limit (> {self.max_gens})"
            )

        self.star.execute_relus_overapprox(
            layer_num, layer_bounds, split_indices, zero_indices
        )
    ...
    def tighten_bounds(
        self, layer_bounds, split_indices, sim, check_cancel_func, depth
    ):
        """
        ...
        if layer_bounds is None:
            ...
            layer_bounds = np.array(
                [[-np.inf, np.inf] for _ in range(num_neurons)], dtype=float
            )
        ...
        if self.do_lp:
            ...
            split_indices = update_bounds_lp(
                layer_bounds,
                self.star,
                sim,
                split_indices,
    ...
    def check_spec(self, spec, check_cancel_func):
        ...
        self.violation_star = spec.get_violation_star(
            self.star, domain_contraction=False
        )
...
class ZonoOverapprox(Freezable):
    ...
    def execute_with_bounds(
        self, _layer_num, layer_bounds, split_indices, zero_indices
    ):
        "do the layer overapproximation with the passed-in bounds"

        if self.get_num_gens() + len(split_indices) > self.max_gens:
            raise OverapproxCanceledException(
                f"{self.type_string} gens exceeds limit (> {self.max_gens})"
            )

        update_zono(
            self.zono, self.relu_update_func, layer_bounds, split_indices, zero_indices
        )
    ...
    def tighten_bounds(
        self, layer_bounds, _split_indices, _sim, _check_cancel_func, _depth
    ):
        """
        update the passed-in layer bounds
...
def update_zono(z, relu_update_func, bounds, splits, zeros):
    ...
    assert (
        len(z.center) == lb_len
    ), "zonotope dims ({len(z.center)}) doesn't match layer_bounds {lb_len}"
    ...
    if splits.size > 0:
        ...
        for i, split_index in enumerate(splits):
            ...
            relu_update_func(
                lb, ub, split_index, gen_mat_t, center, new_generators[:, i]
            )
