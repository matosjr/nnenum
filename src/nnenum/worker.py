...
class Worker(Freezable):
    ...
    def main_loop(self):
        ...
        while not should_exit:
            ...
            if self.priv.ss and self.priv.ss.is_finished(self.shared.network):
                ...
                if self.priv.work_list and Settings.BRANCH_MODE in [
                    Settings.BRANCH_EGO,
                    Settings.BRANCH_EGO_LIGHT,
                ]:
            ...
            try:  # catch lp timeout
                if self.priv.ss and not self.has_timeout():
                    ...
                    if (
                        self.priv.ss
                        and self.priv.ss.prefilter.output_bounds.branching_neurons.size
                        == 0
                    ):
                        self.priv.ss.propagate_up_to_split(
                            self.shared.network, self.priv.start_time
                        )
            ...
            if self.priv.ss is None:
                ...
                if self.priv.work_list:  # pop from local
                    ...
                    if (
                        self.priv.worker_index == 0
                        or self.shared.finished_initial_overapprox.value
                    ):
                    ...
                    if self.priv.ss is not None:
                        ...
                        if self.priv.fulfillment_requested_time is not None:
                            diff = (
                                time.perf_counter()
                                - self.priv.fulfillment_requested_time
                            )
            ...
            # shuffle (optional)
            if (
                Settings.SHUFFLE_TIME is not None
                and time.perf_counter() > self.priv.next_shuffle_time
            ):
    ...
    def add_branch_str(self, label):
        ...
        if self.priv.branch_tuples_list is not None:
            self.priv.branch_tuples_list.append(
    ...
    def consider_overapprox(self):
        ...
        if do_overapprox:
            ...
            if concrete_io_tuple is None:
                ...
                try:
                    gen_limit = max(
                        self.priv.max_approx_gen, Settings.OVERAPPROX_MIN_GEN_LIMIT
                    )
                    ...
                    if res.concrete_io_tuple is not None:
                        ...
                        if Settings.OVERAPPROX_GEN_LIMIT_MULTIPLIER is not None:
                            if is_safe:
                                new_max = (
                                    Settings.OVERAPPROX_GEN_LIMIT_MULTIPLIER
                                    * res.get_max_gens()
                                )
    ...
    def update_shared_variables(self):
        ...
        if self.priv.shared_update_urgent or (
            self.priv.work_list and now > self.priv.next_shared_var_update
        ):
            ...
            self.priv.next_shared_var_update = (
                now + Settings.UPDATE_SHARED_VARS_INTERVAL
            )
            ...
            if did_lock:
                ...
                for i, size in enumerate(self.shared.heap_sizes):
                    if i != self.priv.worker_index:
                        if size == 0:
                            num_zeros += 1
                            self.shared.heap_sizes[
                                i
                            ] = 0  # temp marker so other people don't give them work
                ...
                self.shared.stars_in_progress.value += (
                    self.priv.update_stars_in_progress
                )

                self.shared.incorrect_overapprox_count.value += (
                    self.priv.incorrect_overapprox_count
                )
                self.shared.incorrect_overapprox_time.value += (
                    self.priv.incorrect_overapprox_time
                )
    ...
    def print_progress(self):
        ...
        if self.priv.worker_index == 0:
            ...
            if self.priv.last_print_time is None:
                self.priv.last_print_time = (
                    now - Settings.PRINT_INTERVAL - 1
                )  # force a print at start
            ...
            if (
                ...
                and Settings.PRINT_PROGRESS
                and now - self.priv.last_print_time > Settings.PRINT_INTERVAL
            ):
                ...
                expected_stars = round(
                    1 if finished_frac < 1e-9 else finished / finished_frac
                )

                print(
                    f"({time_str}) Q: {qsize}, Sets: {finished}/{total_stars} "
                    + f" ({round(finished_frac * 100, 3)}%) ETA: {eta} (expected {expected_stars} stars)   ",
                    end="\r",
                )
    ...
    def is_finished_with_lock(self):
        ...
        if not rv and self.shared.had_exception.value == 1:
            ...
            if Settings.PRINT_OUTPUT:
                print(
                    f"Worker {self.priv.worker_index} quitting due to exception in some worker"
                )
    ...
    def clear_remaining_work(self):
        ...
        while not should_exit:
            ...
            if self.shared.stars_in_progress.value <= 0:
                assert (
                    self.shared.stars_in_progress.value == 0
    ...
    def do_load_balancing(self, num_zeros):
        ...
        if self.priv.work_list:
            ...
            if num_zeros > 0:
                while (
                    num_zeros > 0
                    and len(self.priv.work_list) > 1
                    or (len(self.priv.work_list) > 0 and self.priv.ss is not None)
                ):
            ...
            if branch_list_in_branch_tuples(exec_branch_list, branch_tuples):
                ...
                while tighten_factor < 1e16:
                    ...
                    full_cinput = nn_unflatten(
                        full_cinput_flat, self.shared.network.get_input_shape()
                    )
                    exec_output, exec_branch_list = self.shared.network.execute(
    ...
    def finished_star(self):
        ...
        if spec is not None and spec.zono_might_violate_spec(ss.prefilter.zono):
            violation_star = self.shared.spec.get_violation_star(
                ss.star, safe_spec_list=ss.safe_spec_list
            )
            ...
            if violation_star is not None and Settings.FIND_CONCRETE_COUNTEREXAMPLES:
                concrete_io_tuple = self.find_concrete_io(
                    violation_star, ss.branch_tuples
                )
        ...
        # branch_str
        self.add_branch_str(
            f"concrete {'UNSAFE' if violation_star is not None else 'safe'}"
        )
    ...
    def advance_star(self):
        ...
        if not ss.is_finished(network):
            ...
            if (
                new_star
            ):  # new_star can be null if it wasn't really a split (copy prefilter)
