...
class Settings(metaclass=FreezableMeta):
    ...
    BRANCH_OVERAPPROX, BRANCH_EGO, BRANCH_EGO_LIGHT, BRANCH_EXACT = range(
        4
    )  # used for BRANCH_MODE
    ...
    def reset(cls):
        ...
        cls.RESULT_SAVE_POLYS = (
            False  # save 2-d projections of output polygons to Result.polys?
        )
        cls.RESULT_SAVE_POLYS_DIMS = (
            0,
            1,
        )  # (x_dim, y_dim) of 2-d projections, used if RESULT_SAVE_POLYGONS is True

        cls.RESULT_SAVE_STARS = False  # save LpStar objects in result?

        cls.RESULT_SAVE_TIMERS = (
            []
        )  # list of timers to record in Result.timers; TIMING_STATS must be True

        cls.FIND_CONCRETE_COUNTEREXAMPLES = (
            True  # should we try to find concrete counterexamples if spec violated?
        )
        ...
        cls.OVERAPPROX_GEN_LIMIT_MULTIPLIER = (
            1.5  # don't try approx star if multizono.gens > THIS * last_safe_gens
        )
        cls.OVERAPPROX_MIN_GEN_LIMIT = 50  # minimum generators to use as cap
        cls.OVERAPPROX_LP_TIMEOUT = (
            1.0  # timeout for LP part of overapproximation, use np.inf for unbounded
        )
        ...
        cls.SAVE_BRANCH_TUPLES_TIMES = (
            True  # when saving branch tuples, also include runtimes
        )
        cls.BRANCH_MODE = cls.BRANCH_OVERAPPROX
        cls.PRINT_BRANCH_TUPLES = False

        cls.TRY_QUICK_OVERAPPROX = True
        cls.QUICK_OVERAPPROX_TYPES = [
            ["zono.area"],
            ["zono.area", "zono.ybloat", "zono.interval"],
        ]
        ...
        cls.SPLIT_ORDER = (
            cls.SPLIT_ONE_NORM
        )  # rearrange splitting order within each layer

        cls.RESULT_SAVE_POLYS_EPSILON = (
            1e-7  # accuracy of vertices when projecting polygons for Kamenev method
        )
        ...
        cls.TEST_FUNC_BEFORE_ASSIGNMENT = (
            None  # function to call before eager assignement, used for unit testing
        )

        cls.SPLIT_IF_IDLE = True  # force splitting (rather than overapproximation) if there are idle processes

        cls.SHUFFLE_TIME = (
            None  # shuffle star sets after some time (improves unsafe specs)
        )
